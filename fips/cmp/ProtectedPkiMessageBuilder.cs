...
namespace Org.BouncyCastle.Cmp
{
    public class ProtectedPkiMessageBuilder
    {
        ...
        public ProtectedPkiMessage Build(ISignatureFactory<AlgorithmIdentifier> signatureFactory)
        {
            if (null == body)
            {
            ...
            if (signatureFactory.AlgorithmDetails == null)
            {
        ...
        public ProtectedPkiMessage Build(IMacFactory<AlgorithmIdentifier> factory)
        {
            if (null == body)
            {
                throw new InvalidOperationException("body must be set before building");
        ...
        private byte[] CalculateSignature<T>(IStreamCalculator<T> signer, PkiHeader header, PkiBody body)
        {
            ...
            T result = signer.GetResult();

           // if (result is ISigner)
          //  {
          //      return ((IVerifier)result).
          //  }
          //  else
