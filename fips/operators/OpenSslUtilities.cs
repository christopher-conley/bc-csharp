...
namespace Org.BouncyCastle.Operators
{
    class OpenSslUtilities
    {
        ...
        {
            private readonly DekInfo dekInfo;
            private readonly ICipherBuilder<IParameters<Algorithm>> baseCipher;
        ...
        {
            private readonly DekInfo dekInfo;
            private readonly IBlockCipherBuilder<IParameters<Algorithm>> baseCipher;

            internal PemBlockCipherImpl(DekInfo dekInfo, IBlockCipherBuilder<IParameters<Algorithm>> baseCipher)
            {
                this.dekInfo = dekInfo;
                this.baseCipher = baseCipher;
            }
            ...
            byte[] iv)
        {
            ...
            //byte[] sKey;
            IBlockCipherService cipherService;
            ...
            {
                return new PemBlockCipherImpl(dekInfo, blockCipherBuilder);
            }
            else
            {
        ...
        private static byte[] GetKey(char[] password, int keyLength, byte[] salt)
        {
            return GetKey(password, keyLength, salt, false);
        }

        private static byte[] GetKey(char[] password, int keyLength, byte[] salt, bool des2)
        {
