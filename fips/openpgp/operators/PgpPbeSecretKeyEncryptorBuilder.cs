...
namespace Org.BouncyCastle.OpenPgp.Operators
{
    public class PgpPbeSecretKeyEncryptorBuilder
    {
        ...
        public IPbeSecretKeyEncryptor Build(char[] passPhrase)
        {
            ...
            IDigestFactory<PgpDigestTypeIdentifier> checksumCalculatorFactory = null;
            if (SecretKeyPacket.UsageSha1 == checksumUsage)
            {
                checksumCalculatorFactory = new PgpSha1DigestFactory();
            }

            return new SecretKeyEncryptor(encAlgorithm, key, null, s2k, checksumCalculatorFactory);
        }
        ...
        private class SecretKeyEncryptor : IPbeSecretKeyEncryptor
        {
            ...
            internal SecretKeyEncryptor(SymmetricKeyAlgorithmTag symAlg, byte[] key, byte[] iv, S2k s2k,
                IDigestFactory<PgpDigestTypeIdentifier> checksumCalculatorFactory)
            {
                ...
                {
                    parameters = FipsTripleDes.Cfb64.WithIV(new SecureRandom());
                ...
                cipherBuilder = CryptoServicesRegistrar.CreateService(new FipsTripleDes.Key(key))
                    .CreateEncryptorBuilder(parameters);
            }
            ...
            {
                get { return mChecksumCalculatorFactory; }
            }
