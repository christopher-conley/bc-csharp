...
namespace Org.BouncyCastle.Cert
{
    class PemParser
    {
            ...
            Stream inStream)
        {
            ...
            {
                if (PkixCertFunctions.StartsWith(line, _header1) || PkixCertFunctions.StartsWith(line, _header2))
                {
                    break;
                }
            }
            ...
            while ((line = ReadLine(inStream)) != null)
            {
                if (PkixCertFunctions.StartsWith(line, _footer1) || PkixCertFunctions.StartsWith(line, _footer2))
                {
                    break;
                }
