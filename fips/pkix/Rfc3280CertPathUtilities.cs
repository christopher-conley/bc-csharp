    ...
    public class Rfc3280CertPathUtilities
    {
		...
		*/
            ...
            X509Crl crl)
        {
            ...
            try
            {
                idp = IssuingDistributionPoint.GetInstance(Asn1Sequence.GetInstance(
                    crl.GetExtensionValue(X509Extensions.IssuingDistributionPoint))); //    PkixCertPathValidatorUtilities.GetExtensionValue(crl., X509Extensions.IssuingDistributionPoint));
            }
            catch (Exception e)
            {
            ...
            if (idp != null)
            {
                if (idp.DistributionPoint != null)
                {
                    ...
                    IList<GeneralName> names = new List<GeneralName>();
                ...
                try
                {
                    if (cert is X509Certificate)
                    {
                        ...
                        bc = BasicConstraints.GetInstance(
                            crt.GetExtensionValue(X509Extensions.BasicConstraints));
                    }
                    else 
                    {
                        bc = BasicConstraints.GetInstance(
                            ((X509V2AttributeCertificate)cert).GetExtensionValue(X509Extensions.BasicConstraints));
                    }

                    //   PkixCertPathValidatorUtilities.GetExtensionValue(
                       // (X509V2AttributeCertificate)cert, X509Extensions.BasicConstraints));
                }
                catch (Exception e)
                {
        ...
        //throws CertPathValidatorException
        {
            ...
            {
                X500Name principal = cert.SubjectDN;
                ...
                try
                {
                    altName = GeneralNames.GetInstance(cert.GetExtensionValue(X509Extensions.SubjectAlternativeName));
                ...
                // TODO needs review.
                Rdn[] emails = X500Name.GetInstance(dns).GetRdns(BCStyle.EmailAddress);//   GetValueList(X500Name.EmailAddress);
                foreach (Rdn email in emails)
                {
                    GeneralName emailAsGeneralName = new GeneralName(GeneralName.Rfc822Name, email.First.Value);
                    try
                    {
        ...
        //throws CertPathValidatorException
        {
            ...
            try
            {
                pm = Asn1Sequence.GetInstance(cert.GetExtensionValue(X509Extensions.PolicyMappings));
        ...
        //throws CertPathValidatorException
        {
            ...
            try
            {
                certPolicies = DerSequence.GetInstance(cert.GetExtensionValue(X509Extensions.CertificatePolicies));
            ...
            if (certPolicies != null && validPolicyTree != null)
            {
                ...
                if ((inhibitAnyPolicy > 0) || ((i < n) && PkixCertPathValidatorUtilities.IsSelfIssued(cert)))
                {
                    foreach (Asn1Encodable ae in certPolicies)
                    {
                        ...
                        if (Rfc3280CertPathUtilities.ANY_POLICY.Equals(pInfo.PolicyIdentifier.Id))
                        {
                            ...
                            IList<PkixPolicyNode> _nodes = policyNodes[i - 1];
                ...
                {
                    bool critical = criticalExtensionOids.Contains(X509Extensions.CertificatePolicies);

                    IList<PkixPolicyNode> nodes = policyNodes[i];
		...
		*/
        internal static void ProcessCrlB1(
            ...
            X509Crl crl)
        {
            Asn1Object idp =  PkixCertFunctions.NullSafeAsn1Object(crl.GetExtensionValue(X509Extensions.IssuingDistributionPoint));         

            //  PkixCertPathValidatorUtilities.GetExtensionValue(
            //   crl.GetExtensionValue(X509Extensions.IssuingDistributionPoint)); 
            ...
            {
                if (PkixCertFunctions.Equivalent(crl.IssuerDN, PkixCertPathValidatorUtilities.GetIssuerPrincipal(cert),true))
                {
                    matchIssuer = true;
                }
            }
        ...
        //throws AnnotatedException
        {
            ...
            try
            {
                idp = IssuingDistributionPoint.GetInstance(
                    Asn1Sequence.GetInstance(crl.GetExtensionValue(X509Extensions
                        .IssuingDistributionPoint))); //   PkixCertPathValidatorUtilities.GetExtensionValue(crl, X509Extensions.IssuingDistributionPoint));
            }
            catch (Exception e)
            {
		...
		*                            some error occurs.
		*/
        internal static ISet<IAsymmetricKey> ProcessCrlF(
            X509Crl crl,
            object cert,
            X509Certificate defaultCRLSignCert,
            IAsymmetricKey defaultCRLSignKey,
            ...
            IList<X509Certificate> certPathCerts)
        {
            ...
            IList<X509Certificate> coll = Platform.CreateArrayList<X509Certificate>();

            try
            {
                foreach (X509Certificate o in PkixCertPathValidatorUtilities.FindCertificates(selector, paramsPKIX.GetCertStores()))
                {
                    coll.Add(o);
                }
            ...
            IList<X509Certificate> validCerts = new List<X509Certificate>();
            IList<IAsymmetricKey> validKeys = new List<IAsymmetricKey>();
            ...
            ISet<IAsymmetricKey> checkKeys = new HashSet<IAsymmetricKey>();
        ...
        internal static IAsymmetricKey ProcessCrlG(
            X509Crl crl,
            ISet<IAsymmetricKey> keys)
        {
            Exception lastException = null;
            foreach (IAsymmetricKey key in keys)
            {
                try
                {
                    crl.Verify(new PkixVerifierFactoryProvider((IAsymmetricPublicKey)key));
                    return key;
                }
                catch (Exception e)
                {
		...
		*/
        ...
        //throws AnnotatedException
        {
            ...
            while (crl_iter.MoveNext() && certStatus.Status == CertStatus.Unrevoked && !reasonMask.IsAllReasons)
            {
                try
                {
                    ...
                    // (f)
                    ISet<IAsymmetricKey> keys = Rfc3280CertPathUtilities.ProcessCrlF(crl, cert, defaultCRLSignCert, defaultCRLSignKey,
                        paramsPKIX, certPathCerts);
                    // (g)
                    IAsymmetricKey key = Rfc3280CertPathUtilities.ProcessCrlG(crl, keys);
                    ...
                    ISet<DerObjectIdentifier> criticalExtensions = PkixCertFunctions.AsSet<DerObjectIdentifier>(crl.GetCriticalExtensionOids());
                    ...
                    if (deltaCRL != null)
                    {
                        criticalExtensions = PkixCertFunctions.AsSet<DerObjectIdentifier>(deltaCRL.GetCriticalExtensionOids());
                        if (criticalExtensions != null)
                        {
                            criticalExtensions = new HashSet<DerObjectIdentifier>(criticalExtensions);
                            criticalExtensions.Remove(X509Extensions.IssuingDistributionPoint);
            ...
            IList<X509Certificate> certPathCerts)
        {
            ...
            try
            {
                crldp = CrlDistPoint.GetInstance(
                    Asn1Sequence.GetInstance(cert.GetExtensionValue(X509Extensions.CrlDistributionPoints))); //    PkixCertPathValidatorUtilities.GetExtensionValue(cert, X509Extensions.CrlDistributionPoints));
            }
            catch (Exception e)
            {
        ...
        //throws CertPathValidatorException
        {
            ...
            try
            {
                pm = (Asn1Sequence) Asn1Sequence.GetInstance(cert.GetExtensionValue(X509Extensions.PolicyMappings)); //  PkixCertPathValidatorUtilities.GetExtensionValue(cert, X509Extensions.PolicyMappings));
            }
            catch (Exception ex)
            {
                throw new PkixCertPathValidatorException(
                        ...
                        if (!idp_found)
                        {
                            ...
                            while (nodes_i.MoveNext())
                            {
                                ...
                                if (Rfc3280CertPathUtilities.ANY_POLICY.Equals(node.ValidPolicy))
                                {
                                    ...
                                    try
                                    {
                                        policies = (Asn1Sequence) Asn1Sequence.GetInstance(
                                            cert.GetExtensionValue(X509Extensions.CertificatePolicies)); //   PkixCertPathValidatorUtilities.GetExtensionValue(cert,
        ...
        internal static ISet<X509Crl>[] ProcessCrlA1ii(
            ...
            X509Crl crl)
        {
            ISet<X509Crl> deltaSet = new HashSet<X509Crl>();
            X509CrlStoreSelector crlselect = new X509CrlStoreSelector();
            crlselect.CertificateChecking = cert;

            try
            {
                IList<X500Name> issuer = new List<X500Name>();
            ...
            if (paramsPKIX.IsUseDeltasEnabled)
            {
                ...
                try
                {
                    foreach (X509Crl deltaCrl in PkixCertPathValidatorUtilities.GetDeltaCrls(currentDate, paramsPKIX, crl))
                    {
            ...
            X509Crl crl)
        {
            ...
            if (paramsPKIX.IsUseDeltasEnabled)
            {
                ...
                try
                {
                    freshestCRL = CrlDistPoint.GetInstance(Asn1Sequence.GetInstance(  cert.GetExtensionValue(X509Extensions.FreshestCrl)));
                ...
                if (freshestCRL == null)
                {
                    try
                    {
                        freshestCRL = CrlDistPoint.GetInstance(Asn1Sequence.GetInstance(crl.GetExtensionValue(X509Extensions.FreshestCrl)));//   PkixCertPathValidatorUtilities.GetExtensionValue(crl, X509Extensions.FreshestCrl));
                    }
                    catch (Exception e)
                    {
            ...
            X509Certificate sign)
        {
            ...
            try
            {
                ...
                cert.Verify(new PkixVerifierFactoryProvider((IAsymmetricPublicKey)workingPublicKey));
            }
            ...
            // (a) (4) name chaining
            //
            X500Name issuer = PkixCertPathValidatorUtilities.GetIssuerPrincipal(cert);
            if (!PkixCertFunctions.Equivalent(issuer,workingIssuerName,true)) //    issuer.Equivalent(workingIssuerName, true))
            {
            ...
            int explicitPolicy)
        {
            ...
            try
            {
                pc = DerSequence.GetInstance(cert.GetExtensionValue(X509Extensions.PolicyConstraints));
        ...
        //throws CertPathValidatorException
        {
            ...
            try
            {
                Asn1Sequence ncSeq = DerSequence.GetInstance(cert.GetExtensionValue(X509Extensions.NameConstraints));
        ...
        //throws CertPathValidatorException
        {
            ...
            try
            {               
                iap = DerInteger.GetInstance(PkixCertFunctions.NullSafeAsn1Object( cert.GetExtensionValue(X509Extensions.InhibitAnyPolicy)));
        ...
        //throws CertPathValidatorException
        {
            ...
            try
            {
                bc = BasicConstraints.GetInstance(cert.GetExtensionValue(X509Extensions.BasicConstraints));
        ...
        internal static void PrepareNextCertO(
            PkixCertPath certPath,
            int index,
            ISet<DerObjectIdentifier> criticalExtensions,
            ICollection<PkixCertPathChecker> pathCheckers)
        //throws CertPathValidatorException
        {
            IList<X509Certificate> certs = certPath.Certificates;
            X509Certificate cert = (X509Certificate)certs[index];
                                                                                       ...
                                                                                       // (ii)
            {
                if (paramsPKIX.IsExplicitPolicyRequired)
                {
                    ...
                    else
                    {
                        ...
                        {
                            IList<PkixPolicyNode> _nodeDepth = policyNodes[j];
		...
		*/
            ...
            PkixParameters pkixParams)
        {
            ...
            {
                completeidp =
                    IssuingDistributionPoint.GetInstance(Asn1Sequence.GetInstance(
                        completeCRL.GetExtensionValue(X509Extensions.IssuingDistributionPoint)));
            ...
            {
                // (c) (1)
                if (!PkixCertFunctions.Equivalent(deltaCRL.IssuerDN, completeCRL.IssuerDN, true))
                ...
                try
                {
                    deltaidp = IssuingDistributionPoint.GetInstance(
                        deltaCRL.GetExtensionValue(X509Extensions.IssuingDistributionPoint));
                ...
                try
                {
                    completeKeyIdentifier =
                       PkixCertFunctions.NullSafeAsn1Object(
                            completeCRL.GetExtensionValue(X509Extensions
                                .AuthorityKeyIdentifier)); //   PkixCertPathValidatorUtilities.GetExtensionValue(
                ...
                try
                {
                    deltaKeyIdentifier = PkixCertFunctions.NullSafeAsn1Object(deltaCRL.GetExtensionValue(X509Extensions.AuthorityKeyIdentifier));
                //    PkixCertPathValidatorUtilities.GetExtensionValue(
