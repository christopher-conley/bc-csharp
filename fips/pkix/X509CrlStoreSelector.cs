...
using X509Certificate = Org.BouncyCastle.Cert.X509Certificate;
...
namespace Org.BouncyCastle.Pkix
{
    public class X509CrlStoreSelector:ISelector<X509Crl>,ICheckingCertificate
    {
        ...
        private ICollection<X500Name> issuers;
        ...
        /// </summary>
        public ICollection<X500Name> Issuers
        {
            get { return  new List<X500Name>(issuers); }
            set { issuers = PkixCertFunctions.CreateList<X500Name>(value); }
        }
            ...
            X509Crl c)
        {
            ...
            if (issuers != null)
            {
                X500Name i = c.IssuerDN;

                bool found = false;

                foreach (X500Name issuer in issuers)
                {
                    if (PkixCertFunctions.Equivalent(issuer, i, true))
                    {
                        found = true;
                        break;
                    }
            ...
            {
                byte[] extVal = c.GetExtensionValue(X509Extensions.CrlNumber);
                if (extVal == null)
                    return false;

                BigInteger cn = CrlNumber.GetInstance(Asn1Object.FromByteArray(extVal)).PositiveValue;
            ...
            try
            {
                byte[] bytes = c.GetExtensionValue(X509Extensions.DeltaCrlIndicator);
                if (bytes != null)
                {
                    dci = DerInteger.GetInstance(Asn1Object.FromByteArray(bytes));
                }
            }
            catch (Exception)
            {
            ...
            {
                byte[] idp = c.GetExtensionValue(X509Extensions.IssuingDistributionPoint);
