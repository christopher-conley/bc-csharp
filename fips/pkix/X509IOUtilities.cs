...
namespace Org.BouncyCastle.Pkix
{
    public class X509IOUtilities
    {
        ...
        public static X509Crl ParseCRL(object src)
        {
            if (src is byte[])
            {
                return new X509CrlParser().ReadCrl((byte[])src);
            }
            else if (src is Stream)
            {
                return new X509CrlParser().ReadCrl((Stream)src);
            }
            throw new ArgumentException("src must be byte[] or Stream");
        ...
        public static ICollection<X509Crl> ParseCRLs(object src)
        {
            if (src is byte[])
            {
                return new X509CrlParser().ReadCrls((byte[])src);
            }
            else if (src is Stream)
            {
                return new X509CrlParser().ReadCrls((Stream)src);
            }
            throw new ArgumentException("src must be byte[] or Stream");
        }
