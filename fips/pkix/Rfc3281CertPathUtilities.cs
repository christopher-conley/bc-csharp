...
namespace Org.BouncyCastle.Pkix
{
    public class Rfc3281CertPathUtilities
    {
                ...
                PkixParameters pkixParams)
            {
                ...
                // Proxy
                ISet<String> critExtOids =
                    PkixCertFunctions.AsSet<String>(attrCert.GetCriticalExtensionOids(), delegate(object o)
                    {
                        DerObjectIdentifier oid = o as DerObjectIdentifier;
                        if (oid != null)
                        {
                            return oid.Id;
                        }                      
                ...
                if (critExtOids.Contains(X509Extensions.TargetInformation.Id))
                {
                    try
                    {
                        TargetInformation.GetInstance(attrCert.GetExtensionValue(X509Extensions.TargetInformation));
            ...
            */
                ...
                IList<X509Certificate> certPathCerts)
            {
                if (paramsPKIX.IsRevocationEnabled)
                {
                    ...
                    if (attrCert.GetExtensionValue(X509Extensions.NoRevAvail) == null)
                    {
                        ...
                        try
                        {
                            crldp = CrlDistPoint.GetInstance(Asn1Sequence.GetInstance(
                                attrCert.GetExtensionValue(X509Extensions.CrlDistributionPoints)));
                ...
                foreach (TrustAnchor anchor in set)
            {
                ...
                IDictionary<DerObjectIdentifier, string> symbols =   X500NameSymbols.RFC2253Symbols;
                  //  if (acIssuerCert.SubjectDN.   ToString(false, symbols).Equals(anchor.CAName)
                    //    || acIssuerCert.Equals(anchor.TrustedCert))
                    // TODO check.
                        if (acIssuerCert.SubjectDN.Equals(new X500Name(anchor.CAName))
                            || acIssuerCert.Equals(anchor.TrustedCert))
            ...
            */
                ...
                PkixParameters pkixParams)
            {
                ...
                // find holder PKCs
                ISet<X509Certificate> holderPKCs = new HashSet<X509Certificate>();
                if (attrCert.Holder.GetIssuer() != null)
                {
                    X509CertStoreSelector selector = new X509CertStoreSelector();
                    selector.SerialNumber = attrCert.Holder.SerialNumber;
                    X500Name[] principals = attrCert.Holder.GetIssuer();
                    for (int i = 0; i < principals.Length; i++)
                    {
                        try
                        {
                            ...
                            foreach (X509Certificate findCertificate in PkixCertPathValidatorUtilities
                                .FindCertificates(selector, pkixParams.GetCertStores()))
                            {
                                holderPKCs.Add(findCertificate);
                            }
                ...
                {
                    X509CertStoreSelector selector = new X509CertStoreSelector();
                    X500Name[] principals = attrCert.Holder.GetEntityNames();
                    for (int i = 0; i < principals.Length; i++)
                    {
                        try
                        {
            ...
            */
                ...
                IList<X509Certificate> certPathCerts)
            {
                ...
                * getAdditionalStore()
                */
                ISet<X509Crl> crls = PkixCertPathValidatorUtilities.GetCompleteCrls(dp, attrCert,
                       ...
                       && !reasonMask.IsAllReasons)
                {
                    try
                    {
                        ...
                        */
                        ...
                        // (f)
                        ISet<IAsymmetricKey> keys = Rfc3280CertPathUtilities.ProcessCrlF(crl, attrCert,
                            null, null, paramsPKIX, certPathCerts);
                        // (g)
                        IAsymmetricKey pubKey = Rfc3280CertPathUtilities.ProcessCrlG(crl, keys);
