...
namespace Org.BouncyCastle.Pkix
{
    ...
    //		: CertPathBuilderSpi
    {
            ...
            PkixBuilderParameters pkixParams)
        {
            ...
            ISelector<X509Certificate> certSelect = pkixParams.GetTargetCertConstraints();
            if (!(certSelect is X509CertStoreSelector))
            {
                throw new PkixCertPathBuilderException(
                    "TargetConstraints must be an instance of "
                    + typeof(X509CertStoreSelector).FullName + " for "
                    + PkixCertFunctions.GetTypeName(this) + " class.");
            }

            ISet<X509Certificate> targets = new HashSet<X509Certificate>();
            try
            {
                foreach (X509Certificate findCertificate in PkixCertPathValidatorUtilities.FindCertificates((X509CertStoreSelector)certSelect, pkixParams.GetCertStores()))
                {
                    targets.Add(findCertificate);
                }
            ...
            if (targets.Count ==0)
                throw new PkixCertPathBuilderException("No certificate found matching targetConstraint(s).");

            PkixCertPathBuilderResult result = null;
            IList<X509Certificate> certPathList = new List<X509Certificate>();
