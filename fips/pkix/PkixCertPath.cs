...
//		: CertPath
	{
        ...
        static PkixCertPath()
        {
            IList<string> encodings = Platform.CreateArrayList<string>();
            encodings.Add("PkiPath");
            encodings.Add("PEM");
            encodings.Add("PKCS7");
            certPathEncodings = new UnmodifiableListProxy<string>(encodings);
        }
			...
			IList<X509Certificate> certs)
		{
			...
			X500Name issuer = ((X509Certificate)certs[0]).IssuerDN;
			bool okay = true;
			...
			for (int i = 1; i != certs.Count; i++)
			{
				...
				if (X500Name.GetInstance(BCStrictStyle.Instance, issuer).Equivalent(cert.SubjectDN))
				{
            ...
            IList<X509Certificate> retList = Platform.CreateArrayList<X509Certificate>(certs.Count);
            IList<X509Certificate> orig = Platform.CreateArrayList<X509Certificate>(certs);
			...
			for (int i = 0; i < certs.Count; i++)
			{
				...
				X500Name subject = cert.SubjectDN;
				foreach (X509Certificate c in certs)
				{
					if (X500Name.GetInstance(BCStrictStyle.Instance, c.IssuerDN).Equivalent(subject))
					{
						found = true;
						break;
					}
			...
			for (int i = 0; i != retList.Count; i++)
			{
				...
				for (int j = 0; j < certs.Count; j++)
				{
					...
					if (X500Name.GetInstance(BCStrictStyle.Instance, issuer).Equivalent(c.SubjectDN))
					{
						retList.Add(c);
						certs.RemoveAt(j);
						break;
...
//			: base("X.509")
		{
			this.certificates = SortCerts((IList<X509Certificate>)Platform.CreateArrayList(certificates));
		}
...
//			: base("X.509")
		{
			...
			try
			{
				...
				{
                    System.Collections.ICollection cs = new X509CertificateParser().ReadCertificates(inStream);
                    certs = Platform.CreateArrayList<X509Certificate>(cs.Count);
		...
		{
            get { return new UnmodifiableListProxy<string>(certPathEncodings); }
		}
...
//				return false;
			...
			IList<X509Certificate> thisCerts = this.Certificates;
            IList<X509Certificate> otherCerts = other.Certificates;

			if (thisCerts.Count != otherCerts.Count)
				return false;

            IEnumerator<X509Certificate> e1 = thisCerts.GetEnumerator();
            IEnumerator<X509Certificate> e2 = otherCerts.GetEnumerator();
			...
			string encoding)
		{
            ...
            else if (Platform.EqualsIgnoreCase(encoding, "PEM"))
			{
				...
				OpenSslPemWriter pWrt = new OpenSslPemWriter(new StreamWriter(bOut));
		...
		{
            get { return new UnmodifiableListProxy<X509Certificate>(certificates); }
		}
