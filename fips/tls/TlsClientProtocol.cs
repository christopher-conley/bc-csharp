...
namespace Org.BouncyCastle.Tls
{
    ...
    //                    throw new TlsFatalAlert(AlertDescription.illegal_parameter);
             ...
             */
            {
                ...
                {
                    if (!resumedSession && !m_tlsClient.ShouldUseExtendedMasterSecret())
                    {
                        throw new TlsFatalAlert(AlertDescription.handshake_failure);
                    }
                }
    ...
    //        } NewSessionTicket;
        ...
        protected virtual void SendClientHello()
        {
            ...
            // NOT renegotiating
            {
                ...
                if (!ProtocolVersion.IsSupportedTlsVersionClient(earliestVersion) ||
                    !ProtocolVersion.IsSupportedTlsVersionClient(latestVersion))
                {
                    throw new TlsFatalAlert(AlertDescription.internal_error);
                }
